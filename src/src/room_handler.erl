...
%%%===================================================================
init(_Args) ->
  %初始化房间统计数(以世界聊天房间id为起始)
  put(?COUNT, 1),
  %预约创建世界聊天
  create_room(undefined, ?WORLD_ROOM_ID, ?WORLD_ROOM, true),
  {ok, #state{room_table = ets:new(room_table, [set, named_table, {keypos, #room.id}]),
              room_user_table = ets:new(room_user_table, [duplicate_bag, named_table, {keypos, #room_user.user_id}])}}.

handle_call(_Request, _From, State) ->
  {reply, ok, State}.

handle_cast({user_send_message, UserId, RoomId, EncodeMsg}, State) ->
  % 寻找房间
  ...
  {noreply, State};
handle_cast({user_enter_room, UserId, UserPName, RoomId}, State) ->
  % 寻找房间
  case ets:lookup(State#state.room_table, RoomId) of
    [Room] ->
      % 记录用户所在房间
      ...
      socket_server:user_enter_room_callback(UserPName, {ok, Room});
    _ ->
      error_logger:error_msg("User:~p enter room:~p failed, room not found~n", [UserId, RoomId]),
      socket_server:user_enter_room_callback(UserPName, {error, "Room not found"})
  end,
  {noreply, State};
handle_cast({user_exit_room, UserId, UserPName, RoomId}, State) ->
  % 寻找房间
  case ets:match_object(State#state.room_user_table, #room_user{room_id = RoomId, user_id = UserId, _ = '_'}) of
    [RoomUser] ->
      ...
      % 调用回调
      socket_server:user_exit_room_callback(UserPName, {ok, #room{id = RoomUser#room_user.room_id}});
    Other ->
      io:format("Other:~p~n", [Other]),
      socket_server:user_exit_room_callback(UserPName, {error, "User not in the room"})
  end,
  {noreply, State};
handle_cast({create_room, UserPName, RoomId, RoomName, Permanent}, State) ->
  % 获取当前进程字典统计数
  Count = get(?COUNT),
  % 更新进程字典统计数
  put(?COUNT, Count + 1),
  ...
  % 创建房间进程
  case room_supervisor:new_room_server(RoomId, RoomName, RegisterName, Permanent) of
    {ok, _} ->
      % 房间进程创建成功, 插入数据
      ets:insert(State#state.room_table, #room{id = RoomId, name = RoomName, p_name = RegisterName, permanent = Permanent}),
      if
        UserPName /= undefined ->
          socket_server:create_room_callback(UserPName, {ok, #room{id = RoomId, name = RoomName}});
        true -> true
      end;
    Result ->
      % 失败则添加错误日志
      ...
      if
        UserPName /= undefined ->
          socket_server:create_room_callback(UserPName, {error, "Create room failed"});
        true -> true
      end
  end,
  {noreply, State};
handle_cast({delete_room, RoomId}, State) ->
  ets:delete(State#state.room_table, RoomId),
  {noreply, State};
handle_cast({list_rooms, UserPName}, State) ->
  socket_server:list_rooms_callback(UserPName, ets:tab2list(State#state.room_table)),
  {noreply, State};
handle_cast({remove_user_completely, UserId}, State) ->
  % 获取用户所在的所有房间并执行退出
  lists:foreach(fun(RoomUser) ->
    ets:delete(State#state.room_user_table, RoomUser#room_user.user_id),
    catch room_server:delete_user(UserId, RoomUser#room_user.room_p_name) end,
    ets:lookup(State#state.room_user_table, UserId)),
  {noreply, State};
